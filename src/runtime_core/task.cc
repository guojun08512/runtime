...
IecTask::IecTask(const Config::TaskData &data)
    : m_data(data) {
  m_SetSize = 1;
}

void IecTask::ExecuteRange(enki::TaskSetPartition,
                                       uint32_t threadNum) {
  ...
  while (!cfg->ts.GetIsShutdownRequested() && !cfg->stopFlag) {
    ...
    if (isScheduler) {
      ...
      for (auto *entry : dueTasks) {
        ...
        auto tms = std::chrono::duration_cast<std::chrono::milliseconds>(tnow.time_since_epoch()).count();
        std::cout << "[IEC:IO" << threadNum << "] " << entry->data.taskLabel
                  << " trigger@" << tms << "ms"
                  << " interval=" << entry->data.intervalMs
...
void NetworkIOConsumerTask::Execute() {
  ...
  std::cout << "[网络IO] 线程" << threadNum << " | 轮次" << m_data.runId
            << " | 任务" << m_data.taskLabel << " | 状态: RUNNING\n";
